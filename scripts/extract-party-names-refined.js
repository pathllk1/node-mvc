  ...
  if (description.includes('RTGS/')) {
    ...
    if (parts.length >= 4) {
      ...
      if (potentialName && potentialName.length >= 3 && 
          !['DR', 'CR', '999'].includes(potentialName.toUpperCase()) &&
          !potentialName.includes('BKID') &&
          !potentialName.match(/\b(LTD|LIMITED|CORP|BANK)\b/) &&
          !potentialName.match(/^[A-Z]{4}\d+$/) &&
          !potentialName.match(/^\d{10,}$/)) {
        return potentialName;
      }
  ...
  if ((description.includes('StCon-') || description.includes('StUBP-')) && 
    ...
    if (parts.length >= 2) {
      ...
      if (potentialName && potentialName.length >= 2 && 
          ...
          !potentialName.match(/^\d{10,}$/)) { // Exclude long numeric sequences
        return potentialName;
      }
    }
  }
  ...
  if (description.includes('/') && description.length > 10) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (part && part.length >= 3 && 
          ...
          !part.includes('LTD') && !part.includes('LIMITED') && !part.includes('CORP') &&
...
function processCSV() {
  try {
    ...
    console.log('=== REFINED PARTY NAME EXTRACTION COMPLETE ===');
    ...
    console.log('\n=== CORRECTION EXAMPLES ===');
    [137, 141, 151, 231].forEach(lineNum => {
      if (lineNum < lines.length) {
        const parts = lines[lineNum].split(',');
        if (parts.length >= 3) {
          const desc = parts[2] || '';
          const oldExtracted = parts[parts.length - 1] || ''; // Last column
          const newExtracted = extractPartyName(desc);
          if (oldExtracted !== newExtracted) {
            console.log(`Line ${lineNum + 1}: "${desc}"`);
            console.log(`  Before: "${oldExtracted}" -> After: "${newExtracted}"`);
          }
        }
      }
    });
